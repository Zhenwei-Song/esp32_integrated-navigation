...
void initAK8963_2(float *destination)
{
    ...
    res = i2c_write(AK8963_ADDRESS, AK8963_CNTL, 1, 0x00);
    if (res != 0) {
        printf("Error initing AK8963  1");
    }
    delay_ms(10);
    // writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x0F); // Enter Fuse ROM access mode
    res = i2c_write(AK8963_ADDRESS, AK8963_CNTL, 1, 0x0F);
    if (res != 0) {
        printf("Error initing AK8963  2");
    }
    delay_ms(10);
    // readBytes(AK8963_ADDRESS, AK8963_ASAX, 3, &rawData[0]);  // Read the x-, y-, and z-axis calibration values
    i2c_read(AK8963_ADDRESS, AK8963_ASAX, 3, &rawData[0]);
    ...
    // writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x00); // Power down magnetometer
    i2c_write(AK8963_ADDRESS, AK8963_CNTL, 1, 0x00);
    delay_ms(10);
    // Configure the magnetometer for continuous read and highest resolution
    // set Mscale bit 4 to 1 (0) to enable 16 (14) bit resolution in CNTL register,
    // and enable continuous mode data acquisition Mmode (bits [3:0]), 0010 for 8 Hz and 0110 for 100 Hz sample rates
    // writeByte(AK8963_ADDRESS, AK8963_CNTL, 1 << 4 | 2); // Set magnetometer data resolution and sample ODR
    i2c_write(AK8963_ADDRESS, AK8963_CNTL, 1, 1 << 4 | 2);
    delay_ms(10);
}
...
void readMagData(short *destination)
{
    ...
    i2c_read(AK8963_ADDRESS, AK8963_ST1, 1, &tempData[0]);
    if (tempData[0] & 0x01) {
        i2c_read(AK8963_ADDRESS, AK8963_XOUT_L, 7, &rawData[0]);
